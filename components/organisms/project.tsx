...
const Project: FC<ProjectProps> = ({ key, children, ...args }) => (
  <div
    key={key}
  ...
  >
    ...
    <p className="flex h-full">{children}</p>
    <p className="flex flex-wrap gap-4 justify-center">
      {args.stack.map((pill: any, idx: number) => {
        return (
          <Pill src={pill.src} size={22} key={idx}>
            {pill.text}
          </Pill>
        );
      })}
